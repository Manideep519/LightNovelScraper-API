  ...
  (async () => {
    ...
    try {
      const browser = await puppeteer.launch({
        args: [
          `--proxy-server =${options.proxy}:${options.port}`,
      ...
      await page.waitForSelector(
        "#manga-chapters-holder > div.page-content-listing.single-page > div > ul"
      );
      const content = await page.content();
      const $ = cheerio.load(content);
      novelData.name = $("div.post-title h1").html().trim();
      novelData.imageUrl = $(
        "div.profile-manga.summary-layout-1 > div > div > div > div.tab-summary > div.summary_image > a > img"
      ).attr("src");
      ...
      $(
        "#manga-chapters-holder > div.page-content-listing.single-page > div > ul li.wp-manga-chapter a"
      ).each((i, elm) => {
      ...
      $(
        ...
        novelData.chaptersListData[i].chapterLinkName = elm.attribs.href
          .replace(`${baseScrapeURL}`, `${protocol}://${url}/`)
          .slice(0, -1);
      });
      $(
    ...
    try {
      ...
      $("div.item-summary > div.post-title.font-title > h3 > a")
        ...
        .each((i, elm) => {
          novel.push({
            title: elm.data,
            link: elm.parentNode.attribs.href.replace(`${baseScrapeURL}`, `${protocol}://${url}/`),
          });
        });
      ...
      res.send(
        JSON.stringify("Error: something went wrong in getNovel List, or check the url", error)
      );
      res.end();
    }
  })();
